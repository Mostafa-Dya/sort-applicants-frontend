...
export class PublicEntitieService {
  ...
  getPublicEntitieDataId(public_entitie_id: number): Observable<any> {
    return this.http.get<any>(
      `${environment.api}public-entities-get/${public_entitie_id}`,
    );
  }

  createPublicEntitieData(data): Observable<any> {
    return this.http.post<any>(
      environment.api + 'create-public-entities',
      data,
    );
  }
  ...
  updatePublicEntitieData(public_entitie_id: number, data): Observable<any> {
    return this.http.put<any>(
      `${environment.api}update-public-entities/${public_entitie_id}`,
      data,
    );
  }
  ...
  deletePublicEntity(data): Observable<any> {
    ...
    if (data.level === 0) {
      ...
      return this.http.delete<any>(`${environment.api}affiliated-entities`, {
        params,
      });
    } else if (data.level === 3) {
      // Node is an affiliated entity
      params = new HttpParams().set(
        'sub_affiliated_entity_ids[]',
        data.id.toString(),
      ); // Convert to string if necessary
      return this.http.delete<any>(
        `${environment.api}sub-affiliated-entities`,
        { params },
      );
    } else {
